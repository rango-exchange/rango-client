...
import { confirmAllowanceOnChain } from './utils';
...
>(
  ...
  const namespace = context
    .hubProvider(sourceWallet.walletType)
    .get(approveAdapter.namespaceKey);
  if (!namespace) {
    handleErr(
      new Err({
        nextStatus: 'failed',
  ...
  try {
    ...
    /*
     * Every failure here - the status lookup, the allowance re-check, a
     * transient RPC error - otherwise becomes an indefinite silent retry, which
     * is indistinguishable from the transaction still being pending.
     */
    console.debug('[approve-status] poll threw, retrying', error);
    await retryAfterDelay();
  }
}
