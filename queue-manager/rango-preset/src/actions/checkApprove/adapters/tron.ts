  ...
  buildApproveTransaction: async (
    ...
    try {
      ...
      return Ok<TronTransaction>({
         ...
         * allowance can re-issue it. Dropping it here would leave the same empty
         * payload that stops TronLink from signing the approve at all.
         */
        __payload__: builtTransaction.__payload__ ?? {},
      });
    } catch {
      return new Err({
        nextStatus: 'failed',
