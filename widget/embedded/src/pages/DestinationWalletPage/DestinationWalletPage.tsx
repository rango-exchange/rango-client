...
export function DestinationWalletsPage() {
  const { toBlockchain, customDestination, setCustomDestination } =
    useQuoteStore();
  const destinationWallet = useAppStore().destinationWallet();
  const {
    connectedWallets,
  ...
  useEffect(() => {
    ...
    if (destinationWallet && !('WalletType' in destinationWallet)) {
  ...
  const handleClear = () => {
    resetState();
    setDestinationWallet(null, connectedWallets);
  };
  ...
  const handleConfirmWallets = () => {
    ...
    const invalidAddress =
      destination &&
      destination.address !== '' &&
      !('walletType' in destinationWallet) &&
          ...
          <Container>
            ...
            <div>
              ...
              <TextField
                ...
                suffix={renderSuffix()}
                value={
                  destinationWallet && !('walletType' in destinationWallet)
                    ? destination?.address ?? ''
                    : ''
                }
                onChange={(event) => {
