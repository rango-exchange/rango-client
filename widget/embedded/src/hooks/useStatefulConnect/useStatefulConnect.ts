...
export function useStatefulConnect(): UseStatefulConnect {
  ...
  const handleConnect = async (
    ...
    const forcedNamespaces = options?.forcedNamespaces;
    ...
    if (isDisconnected) {
      ...
      if (forcedNamespaces) {
        const notAvailableForcedNamespace = forcedNamespaces.find(
          (namespace) =>
        ...
        if (forcedNamespaces.length > 1 && needsDerivationPath) {
          throw new Error(
        ...
        return await runConnect(
          wallet,
          forcedNamespaces.map((namespace) => ({ namespace })),
     ...
     */
    if (!!wallet.isHub && !options?.forcedNamespaces) {
  ...
  const handleDerivationPath = async (
    ...
    const forcedNamespacesEnabled = !!options?.forcedNamespaces;
    // If forced namespaces are enabled, we should bypass detached modal and go directly to connect.
    if (namespaceIsAvailable && !forcedNamespacesEnabled) {
      dispatch({
        type: 'detached',
        payload: {
          targetWallet: wallet,
