...
export function walletAndSupportedChainsNames(supportedChains: ProviderMeta[]): Network[] | null {
...
export function getSelectableWallets(
  ...
  const selectableWallets = connectedWallets.map((connectedWallet: ConnectedWallet) => {
    return {
        ...
        (wallet) =>
          wallet.balances?.find(
            (balance) =>
              ...
              (address === null && balance.address === address && balance.symbol === symbol),
          ) || null,
      )
...
export function isAccountAndWalletMatched(account: Wallet, connectedWallet: ConnectedWallet) {
...
export function resetConnectedWalletState(connectedWallet: ConnectedWallet): ConnectedWallet {
...
export const sortTokens = (tokens: TokenWithBalance[]): TokenWithBalance[] => {
  ...
  return tokens
    ...
    .sort(
      (tokenA, tokenB) =>
        parseFloat(tokenB.balance?.usdValue || '0') - parseFloat(tokenA.balance?.usdValue || '0'),
    )
    .concat(
...
export const isExperimentalChain = (blockchains: ProviderMeta[], wallet: string): boolean => {
...
export function sortWalletsBasedOnState(wallets: ModalWalletInfo[]): ModalWalletInfo[] {
  return wallets.sort(
    (a, b) =>
      Number(b.state === WalletStatus.CONNECTED) - Number(a.state === WalletStatus.CONNECTED) ||
      Number(b.state === WalletStatus.DISCONNECTED || b.state === WalletStatus.CONNECTING) -
        Number(a.state === WalletStatus.DISCONNECTED || a.state === WalletStatus.CONNECTING),
  );
}
