...
import { getBlockChainNameFromId, Network, EvmTransaction, Meta } from '../rango';
...
export async function executeEvmTransaction(
  ...
  if (chainId) {
    ...
    throw new WalletError(WalletErrorCode.UNEXPECTED_BEHAVIOUR, `ChainId ${chainId} is null!`);
  }
  try {
    let tx = {};
    ...
    if (!!evmTransaction.gasPrice)
      tx = { ...tx, gasPrice: '0x' + parseInt(evmTransaction.gasPrice).toString(16) };
  ...
  // @ts-ignore
    ...
    } else if (error && error?.code && error?.code === -32603 && error.message) {
      throw new WalletError(WalletErrorCode.SEND_TX_ERROR, undefined, error.message);
    }
