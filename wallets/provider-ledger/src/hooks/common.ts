...
import { getLedgerError } from '../utils.js';
...
function convertLedgerRejectionError(
  _context: unknown,
  error: unknown
): unknown {
  if (
...
function convertLedgerDeviceError(_context: unknown, error: unknown): unknown {
  return getLedgerError(error);
}

export const commonHooks = {
  convertLedgerRejectionError,
  convertLedgerDeviceError,
};
