...
export const executeCosmosTransaction = async (
  ...
  try {
    ...
    if (signType === 'AMINO') {
      ...
      let signResponse;
      try {
        signResponse = await cosmosProvider.request<CosmosRpcResponse>({
          method: 'cosmos_signAmino',
          params: {
            signDoc,
            signerAddress: cosmosTx.fromWalletAddress,
          },
        });
      } catch (err) {
      ...
      if (cosmosTx.data.protoMsgs.length > 0) {
        signedTx = cosmos.tx.v1beta1.TxRaw.encode({
            ...
            signerInfos: [
              {
                publicKey: {
                  ...
                  value: cosmos.crypto.secp256k1.PubKey.encode({
                    key: Buffer.from(signResponse.pub_key.value, 'base64'),
                  }).finish(),
                },
                modeInfo: {
                  single: {
          ...
          signatures: [Buffer.from(signResponse.signature, 'base64')],
        }).finish();
      } else {
        try {
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          signedTx = makeStdTx(signDoc, signResponse.signature);
        } catch (err) {
          throw new SignerError(SignerErrorCode.SIGN_TX_ERROR, undefined, err);
        }
      }
